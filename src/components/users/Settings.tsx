...
import { updateNombre } from '../../store/slices/auth';
...
export const Settings = () => {
  ...
  const { values, handleInputChange } = useForm<FormData>({ nombre: usuario!.nombre });
  const { nombre } = values;
  ...
  const onUpdateNombre = async () => {

   

    try {
      const { data } = await chatApi.patch('/auth/username',{nombre});
      ...
      <Box display="flex" my={3}>
        ...
        <Box>
          ...
          <Typography variant="subtitle2">
            {usuario!.nombre}, this could be your user settings panel.
          </Typography>
        </Box>
      </Box>
              ...
              <CardContent sx={{ p: 4 }}>
                <Typography variant="subtitle2">
                  <Grid container spacing={0}>
                    ...
                    <Grid item xs={12} sm={8} md={9} mt={1}>
                      <Button variant='contained' onClick={onUpdateNombre}>Change</Button>
